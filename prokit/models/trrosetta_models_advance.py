...
class TrRosettaModelKL(BaseModel):
    ...
    def build_model_zsh_away(self):
        ...
        with tf.name_scope("loss"):
            ...
            # total loss
#             self.loss = (self.loss_dist + self.loss_omega + self.loss_theta + self.loss_phi)/8 + aa_weight*self.loss_aa
            self.loss = (self.loss_dist_bkg + self.loss_omega_bkg + self.loss_theta_bkg + self.loss_phi_bkg)/(len(ref)+1) + aa_weight*self.loss_aa
            
    def formula(self,a,b):
        return -tf.math.reduce_mean(tf.math.reduce_sum(a*tf.math.log(a/b),axis=-1))
    
    def build_model_zsh_close(self):
...
class TrRosettaModelKL_close(BaseModel):
    def __init__(self, config):
        super(TrRosettaModelKL, self).__init__(config)
        self.build_model_close()
