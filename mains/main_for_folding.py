...
#     create_dirs([config.summary_dir, config.checkpoint_dir, config.output_dir])
    print("create dirs")
    print([config.summary_dir, config.checkpoint_dir, config.output_dir])
    files = os.listdir(config.output_dir)
    all_files = []
    dfList = []

    for file in files:
        all_files.append(os.path.join(config.output_dir,file))
    for file in all_files:
        if file.endswith('.csv'):
            df = pd.read_csv(file, index_col=None, header=0)
#             print(re.search(r'\d+', file[-8:]).group(0))
            for i in range(402):
                if (i-1)%20==0:
#                     print(df.iloc[i])
                    process1(config,config_dict,df['sequence'][i],re.search(r'\d+', file[-8:]).group(0))
                    
                
#             dfList.append(df)

#     dfTable = pd.concat(dfList, axis=0, ignore_index=True)
    sys.exit(0)
    print(dfTable)
    
    dfTable = dfTable[dfTable['score']<-3]
    print(dfTable)
    j = 0
    sequences = []
    ...
    for sequence in sequences:
        process(config,config_dict,sequence)
    sys.exit(0)
    
    
def process1(config,config_dict,sequence,seq_id):
    count = len(glob.glob(f"{config.output_dir}/new_final_seq_{seq_id}_*.npz"))
    output_fasta = os.path.join(config.output_dir, f"new_final_seq_{seq_id}_{count}.fasta")
    output_npz = os.path.join(config.output_dir, f"new_final_seq_{seq_id}_{count}.npz")
    output_model = os.path.join(config.output_dir, f"new_final_seq_{seq_id}_{count}_model.pdb")
...
def process(config,config_dict,sequence):
    create_dirs([config.summary_dir, config.checkpoint_dir, config.output_dir])
    count = len(glob.glob(f"{config.output_dir}/new_final_seq_*.npz"))
    output_fasta = os.path.join(config.output_dir, f"new_final_seq_{count}.fasta")
    output_npz = os.path.join(config.output_dir, f"new_final_seq_{count}.npz")
    output_model = os.path.join(config.output_dir, f"new_final_seq_{count}_model.pdb")
