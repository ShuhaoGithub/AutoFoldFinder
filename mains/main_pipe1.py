...
def main():
    ...
    config.npz_list = ['../data/experiments/exp4/output/final_seq_160.npz','../data/experiments/exp4/output/final_seq_717.npz','../data/experiments/exp4/output/final_seq_781.npz','../data/experiments/exp3/output/final_seq_959.npz','../data/experiments/exp4/output/final_seq_621.npz',]
    config.weight_list = [1,1,1,1,1]
    trajectory, finalSeq = run_optimizer_with_trained_model(config, gpuConfig, randIdxArray)
    
    if output_csv != "":
        df = pd.DataFrame(trajectory, columns = ['step', 'sequence', 'score','loss_dist', 'loss_omega', 'loss_theta', 'loss_phi', 'loss_aa','loss_beta','loss_map'])
